...
namespace Forge3D
{
    public class F3DPoolOld : MonoBehaviour
    {
        ...
        public static F3DPoolOld instance; // Singleton instance

        [Header("VFX Pool")] public Transform[] poolItems; // Effect pool prefabs
        public int[] poolLength; // Effect pool items count

        [Header("Audio Pool")] public Transform audioSourcePrefab; // Audio source prefab
                ...
                for (int i = 0; i < audioPoolItems.Length; i++)
                {
                    ...
                    for (int x = 0; x < audioPoolLength[i]; x++)
                    {
                        AudioSource newAudio =
                            ((Transform) Instantiate(audioSourcePrefab, Vector3.zero, Quaternion.identity))
