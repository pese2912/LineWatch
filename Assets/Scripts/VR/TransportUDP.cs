...
public class TransportUDP : MonoBehaviour {
	...
	// 타임아웃 시간.
	private const int 		m_timeOutSec = 5000;
	...
	public bool Connect(string address, int port)
	{
		...
		try {
			if (m_socket == null) {
				serverEP = new IPEndPoint(IPAddress.Parse(address), port);
	...
	void DispatchSend()
	{
		...
		try {
			...
			if (m_socket.Poll(0, SelectMode.SelectWrite)) {
				...
				while (sendSize > 0) {
					int res = m_socket.SendTo(buffer, sendSize, SocketFlags.None, serverEP);	
	...
	void DispatchReceive()
	{
		...
		try {
			while (m_socket.Poll(0, SelectMode.SelectRead)) {
				...
				int recvSize = m_socket.ReceiveFrom(buffer, buffer.Length, SocketFlags.None, ref remoteEP);
